...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function enhanceCardsTouch() {
    const cards = document.querySelectorAll(
      '.seminar-card, .league-card, .track-card, .achievement-card'
    );
  ...
  function initTouchSupport() {
    ...
    const lbContainer =
      document.querySelector('.lb-rows-container') || document.querySelector('.table-wrapper');
    if (lbContainer) {
  ...
  const buttons = document.querySelectorAll(
    '.btn-cyber, .btn-primary, .btn-secondary, .btn-hollow'
  );
  buttons.forEach(btn => {
