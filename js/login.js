document.addEventListener('DOMContentLoaded', () => {
  ...
  function getAccounts() {
    ...
    const uniqueUserAccounts = userStoredAccounts.filter(
      userAcc => !baseCodenameSet.has(userAcc.codename)
    );
  ...
  function checkCodenameOrEmailExists(codename, email) {
    ...
    return accounts.some(
      account => account.codename === lowerCodename || account.email === lowerEmail
    );
  }
  ...
  function startAuthOrRegistrationSimulation(isLogin) {
    ...
    verificationTitle.textContent = isLogin
      ? 'INITIATING ACCESS PROTOCOL...'
      : 'SECURE AGENT HASHING...';
    verificationMessage.textContent =
      'Analyzing credentials against known hashes and security kernels.';
    ...
    setTimeout(
      () => {
        ...
        if (isEmailFound) {
          ...
          verificationMessage.textContent =
            'The entered email key does not match any known records. Please verify the address and try again.';
  ...
  function handleSuccess(isLogin) {
    ...
    verificationMessage.textContent = isLogin
      ? 'Welcome back, Agent. Redirecting to Profile...'
      : 'Agent ID successfully created. Redirecting to Profile...';
  ...
  authForm.addEventListener('submit', e => {
    e.preventDefault();
    let formValid = true;
    const currentView = loginView.classList.contains('active')
      ? 'login'
      : signupView.classList.contains('active')
        ? 'signup'
        : forgotPasswordView.classList.contains('active')
          ? 'forgot'
          : null;
    ...
    if (currentView === 'login') {
      ...
      formValid &= validateField(
        resetEmailInput,
        'reset-email-error',
        'Email Key is required for recovery'
      );
  ...
  function initMatrixRain() {
    ...
    const katakana =
      'アァカサタナハマヤラワガザダバパピビヂゾブヅエェケセテネヘメレヱゲゼデベペオォコソトノホモヨロヲゴゾドボポ10100101';
    const fontSize = 16;
  ...
  function initHackerStats() {
    ...
    function updateHackerStats() {
      ...
      currentStats = currentStats.map(stat => {
        ...
        let displayValue =
          stat.decimal !== undefined ? newValue.toFixed(stat.decimal) : Math.round(newValue);
