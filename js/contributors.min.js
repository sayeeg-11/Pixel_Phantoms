...
async function fetchGlobalActivity() {
  ...
  try {
    ...
    if (statsLabel) statsLabel.textContent = `Collective System Pulse: ${totalYearly} contributions detected in the last solar year.`;
    ...
    try {
      ...
      if (!partData.all || !Array.isArray(partData.all)) throw new Error('Invalid participation data.');
      ...
      if (statsLabel) statsLabel.textContent = `Collective System Pulse: ${totalPart} commits detected via Participation Index.`;
...
function renderHeatmap(data) {
  ...
  data.forEach((week, weekIndex) => {
    ...
    if (!seenMonths.has(monthKey) && monthContainer) {
      ...
      console.log(`📌 "${monthName}" at column ${columnIndex} (${columnIndex * 18}px) for ${weekStartDate.toDateString()}`);
    }
    ...
    week.days.forEach((count, dayIndex) => {
      ...
      cell.setAttribute('data-tooltip', `${count} ${count === 1 ? 'contribution' : 'contributions'} on ${dateStr}`);
...
function loadMockHeatmap() {
  ...
  for (let i = 51; i >= 0; i--) {
    ...
    console.log(`Week ${52-i}: ${weekStart.toDateString()} (${weekStart.getMonth() + 1}/${weekStart.getDate()})`);
