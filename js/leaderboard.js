...
const ACHIEVEMENTS = [
  ...
  },
  {
    id: 'high_complexity',
    name: 'Complex Solver',
    description: 'Submitted a Level 3 PR',
    icon: 'fas fa-brain',
    xp: 300,
  },
  {
  ...
  },
  {
    id: 'speed_demon',
    name: 'Speed Demon',
    description: 'Merged PR within 24 hours',
    icon: 'fas fa-bolt',
    xp: 200,
  },
  {
    ...
    description: 'PR with no review comments',
...
async function initDashboard() {
  ...
  if (tableBody)
    tableBody.innerHTML =
      '<tr><td colspan="6" class="loading-text">INITIALIZING DATA STREAMS...</td></tr>';
...
function renderLeaderboardTable(data) {
  ...
  data.slice(0, 50).forEach(agent => {
    ...
    let classColor =
      agent.class === 'TITAN' ? '#ff0055' : agent.class === 'STRIKER' ? '#ffd700' : '#00f3ff';
...
function renderUserAchievements(achievements) {
  ...
  Object.keys(achievements).forEach(achId => {
    if (achievements[achId]) {
      ...
      if (a)
        container.innerHTML += `<div class="modal-achievement"><i class="${a.icon}"></i><p>${a.name}</p></div>`;
    }
  });
}
...
function loadProfileData() {
  ...
  if (savedData) {
    ...
    if (document.getElementById('p-college'))
      document.getElementById('p-college').value = data.college || '';
    if (document.getElementById('p-grad-year'))
    ...
    if (document.getElementById('p-github'))
      document.getElementById('p-github').value = data.github || '';
    if (document.getElementById('p-linkedin'))
      document.getElementById('p-linkedin').value = data.linkedin || '';
    if (document.getElementById('p-portfolio'))
